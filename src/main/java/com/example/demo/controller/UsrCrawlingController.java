...
public class UsrCrawlingController {
	...
	// "YourPersistenceUnitName"; // JPA (Java Persistence API)에서 사용되는 영속성 단위의 이름
	...
	public void crawl() {
		...
		WebDriver driver = new ChromeDriver(options); // 웹 드라이버 초기화
		...
		try {
			...
			WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(30)); // 웹 페이지 로드를 위한 대기
		...
		try (BufferedWriter writer = new BufferedWriter(
			...
			for (int i = 1; i < elements.size(); i++) {
				for (int j = 1; j <= 9; j++) {
					...
					WebElement text = title.findElement(By.xpath(
							"//*[@id=\"root\"]/article/div/article/section/article/section[2]/article/section[2]/table/tbody/tr["
									+ i + "]/td[" + j + "]"));
					String t = text.getText().replace("\n", ",").replace(")", "),").replace("여자", ",여자,")
							.replace("19", ",19").replace("16", ",16").replace(":00", ":00,").replace(":30", ":30,")
							.replace("육관", "육관,").replace("움", "움,").replace("ER", "ER,").replace("nd", "nd,");
